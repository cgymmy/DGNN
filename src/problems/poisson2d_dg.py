...
from testfuncs.testfunc2d import TestFunction2D


class Poisson2d_dg:
    def __init__(self, boundary_type: str = 'regular', Nint_elt: int = 15, Nint_edge: int = 20,
                 order: int = 3, partition: float = 0.2,
        ...
        # case 2
            ...
            self.f = lambda x, y: torch.ones_like(x) * 10
            self.fxy = self.f(self.elt_int[..., 0], self.elt_int[..., 1])
            self.u_exact = None  # irregular 区域无参考精确解

        self.v_elt_inner, self.v_elt_bd, self.dv_elt_inner = self.get_test(order=order)

        self.model = DGNet(num_modules=self.Nelt, input_size=2, hidden_size=hidden_size, output_size=1,
                           num_layers=num_layers, act=act).to(device).to(self.torch_type)

        self.Lfbgs = torch.optim.LBFGS(self.model.parameters(), lr=1.0, max_iter=30000,
    ...
    def loss(self):
        ...
        if self.u_exact is not None:
            ...
            mse = torch.tensor(float('nan'), device=device)
            mae = torch.tensor(float('nan'), device=device)
        return loss, mse, mae
