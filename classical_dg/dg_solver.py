...
class QuadratureRule:
    ...
    def triangle_gauss_2d(order: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        """
        from src.mesh.triangle_gauss import rule
        return rule(order)
...
class TestFunction:
    ...
    def evaluate_1d(self, x: torch.Tensor, order: int,
        ...
        if self.basis_type == 'polynomial':
            ...
            v_np = legendre_poly(xi_np)
            v = torch.tensor(v_np, dtype=xi.dtype, device=xi.device)
            # 使用Legendre多项式的导数公式: d/dx P_n(x) 可由 P_{n-1} 和 P_n 计算
            if order == 0:
                dv = torch.zeros_like(xi)
            else:
                legendre_deriv = legendre(order - 1)
                dv_np = order * legendre_deriv(xi_np)
                dv = torch.tensor(dv_np, dtype=xi.dtype, device=xi.device) * 2 / h
        else:
    ...
    def evaluate_2d(self, points: torch.Tensor, order: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        grad_list = [torch.zeros(*x.shape, 2, dtype=x.dtype, device=x.device)]

        for i in range(1, order + 1):
            for j in range(i + 1):
                k = i - j
                basis_list.append(x ** j * y ** k)
                grad_x = j * x ** (j - 1) * y ** k if j > 0 else torch.zeros_like(x)
                grad_y = x ** j * k * y ** (k - 1) if k > 0 else torch.zeros_like(y)
...
class DGSolver1D(ABC):
    ...
    def _setup_test_functions(self):
        ...
        self.v_basis = torch.stack(v_list, dim=0)  # (n_basis, n_elem, n_quad+2)
        self.dv_basis = torch.stack(dv_list, dim=0)  # (n_basis, n_elem, n_quad+2)
...
class Poisson1D_DG(DGSolver1D):
    ...
    def compute_loss(self, u: torch.Tensor) -> torch.Tensor:
        ...
        x_interior = self.x_mesh[:, 1:-1]
        f_interior = self.source_term(x_interior)
...
class Poisson2D_DG:
    ...
    def __init__(self, boundary_type: str = 'polygon', n_int_elt: int = 15,
        ...
        try:
            from src.mesh.mesh2d import GenMesh2D
            ...
            raise ImportError("需要src.mesh.mesh2d模块")
...
def demo_1d_poisson():
    ...
    u_test = torch.randn(solver.n_elements, solver.n_quad_points + 2, device=device, dtype=solver.dtype)
